...
class SmallRecipeFavorite extends StatefulWidget {
  const SmallRecipeFavorite({
    Key? key,
  }) : super(key: key);

  @override
  SmallRecipeFavoriteState createState() => SmallRecipeFavoriteState();
}

class SmallRecipeFavoriteState extends State<SmallRecipeFavorite> {
  late List<RecipeCardModel> recipes = <RecipeCardModel>[];

  @override
  void initState() {
    Future.delayed(Duration.zero, () async {
      await FavoriteRecipeService.getFavoriteRecipesByUserId().then((data) {
        setState(() {
          recipes = data;
        });
      });
